...
Plans as sequences of actions can be notated in a colon-seperated manner like this: ```action1:action2:action3:...```. For instance, the flowers domain could also instead have the plan ```plan(give_flowers:wait:give_flowers).```, which denotes the action sequence when the agent first gives flowers, then waits one time step, and then gives flowers again.
